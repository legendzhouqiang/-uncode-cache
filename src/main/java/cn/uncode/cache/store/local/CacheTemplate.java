...
import cn.uncode.cache.framework.CacheSync;
import cn.uncode.cache.framework.Command;
import cn.uncode.cache.framework.ICache.Level;
import cn.uncode.cache.framework.ICache.Operator;
import cn.uncode.cache.framework.RedisPubSubSync;
import cn.uncode.cache.framework.util.NetUtil;
...
public class CacheTemplate {
    ...
    // 配置项
    // ---------------------------------------------------------------------------------------------------------------------------
    private String                                           key                                  = "uncode-cache";
    ...
    public CacheTemplate(JedisTemplate jedisTemplate){
        ...
        if (this.localEnabled) {
            ...
            dsc.setPath(localStoreLocation + ID);
     ...
     * 删除单个缓存值(根据缓存级别)
     */
    public void del(String name, String key, Level level) {
        if (level.equals(Level.Local)) {
            if (!localEnabled) {
                return;
            }
     ...
     */
    public void rem(String name, Level level) {
        if (level.equals(Level.Local)) {
            if (!localEnabled) {
                return;
            }
    ...
    public CacheData getCacheData(String name, String key, Level level) {
        String value = String.valueOf(this.get(name, key, level));
        int tti = -1;
    ...
    private String getRedisKeyOfCache(String name) {
        return CACHE_STORE + spliter + name;
    }

    private String getRedisKeyOfElement(String name, String key) {
        return CACHE_STORE + spliter + name + spliter + key;
    }
